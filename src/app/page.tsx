...
// const INSTAGRAM_AUTH_URL = `http://localhost:3000/test`

function ConnectInstagramButton({ onConnect }: { onConnect: () => void }) {
  return (
    <button
...
function UserDashboard({ userData }: { userData: InstagramUserData }) {
  return (
...
export default function Home() {
  ...
  const handleConnectInstagram = useCallback(() => {
    ...
    // Using ref so we can remove the listener if necessary
    function handleMessage(event: MessageEvent) {
      // Accept only Instagram OAuth messages
      if (
        event.data &&
        ...
        // Manage account after child window sends payload
        const { access_token, token_type, expires_in, scope, instagram_user_id, username, name } = event.data.payload;
        setUserData({
          access_token,
          token_type,
          expires_in,
